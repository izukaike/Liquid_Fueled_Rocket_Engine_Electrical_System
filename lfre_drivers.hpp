 ...
 *  this file file has memoey mapped gpio, 10 bit analog read, and reading aand writing
 *  to adn from eeprom (electrically erasable programmable read only memory) it is non volatile
 *  so it is good for hardcoding tests(max is 1023 bytes so testing is limited), and serial read
 *  and write drivers are writte here. They are not implemented into the main code yet.
 *  
 *  Features:
 *  - C/C++ GPIO Driver
 *  - C/C++ 10-Bit Analog Read Driver
 *  - C/C++ UART Driver
 *  - C/C++ EEPROM Driver
 *  - Non-Volatile Test File Data
 *  - Non-Volatile System Data For Continous Status Updates
 *  
 *  
 *  Work:
 *  
...
struct eeprom
{
  public:
    ...
    eeprom(bool write_test)
    {
      ...
      if(write_test == true)
      {
         ...
         for(int i = 0; i < samples;++i){
            ...
            /*
            Serial.println(this->head_write);
            Serial.println(this->max_addy);
            Serial.println(this->num);
            */
            this->write_(mem_val);
         }
      }
    ...
    /*
     * writes to eeprom. to use 10 bit numbers we truncate 16-bit into 8 bits
     * 
     * takes in val
     * 
   ...
   private:
    ...
    int psi_to_mem(int p,int ducer)
    {
      ...
      //floor function uses because read/write slows down a lot when value is too big
 ...
 */
int  GPIODriver::analog_read_ten(int pin)
{
